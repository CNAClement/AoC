...
if partie=="partie1":
    ...
    for indice in range(0,len(seeds_split),2):  #on utilise un pas de 2
        ...
        for seed in range(  int(seeds_split[indice]),   int(seeds_split[indice])+int(seeds_split[indice+1]), 100000 ): #on fait un premier passage grossier pour identifier la graine autour de laquelle "ça se passe"
            seed = int(seed)
            nombre_mappé = seed
            for map in liste_map:
                nombre_mappé = mapping(nombre_mappé, map)
            ...
            localisation_grossiere=min(localisations)[0]
            numero_grossier_graine=min(localisations)[1]
            #print(numero_grossier_graine)
    for seed in range (numero_grossier_graine - 200000,numero_grossier_graine+200000,500): #on affine en cherchant autour de cette graine
        seed = int(seed)
        nombre_mappé = seed
        for map in liste_map:
            nombre_mappé = mapping(nombre_mappé, map)
        # print(f"la localisation est : {nombre_mappé}")
        localisations.append((nombre_mappé, seed))
        localisation_grossiere2 = min(localisations)[0]
        numero_grossier_graine2 = min(localisations)[1]
    for seed in range(numero_grossier_graine2 - 20000, numero_grossier_graine2+ 20000,1): #on affine encore
        seed = int(seed)
        nombre_mappé = seed
        for map in liste_map:
            nombre_mappé = mapping(nombre_mappé, map)
